...
class Cylinder {
public:
    Cylinder(const std::string& filepath);

    Cylinder(glm::vec3 position, float r, float h);

    Cylinder(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices);
...
protected:
    // vertices of the table represented in Cylinder's own coordinate
